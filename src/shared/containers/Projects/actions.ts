...
import { CompanyFormsModal } from './Modals/CompanyProjectsModal/CompanyProjectsModal';
...
export const MY_FORMS = 'MY_FORMS';
...
interface ActionTypes {
  ...
  MY_FORMS: Array<CompanyFormsModal>;
...
export const listUserProjects =
  ...
  export const listCompanyForms =
  (companyId: number) =>
  async (dispatch: any, _getState = null, utils: any) => {
    const response = await handleApiRequest(
      dispatch,
      utils.Api.get(`/companies/${companyId}/contract-forms`, {}), FETCHING_MY_PROJECTS);
    dispatch({
      type: MY_FORMS,
      payload: response,
    });
