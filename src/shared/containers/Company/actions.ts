import { FETCHING_MY_PROJECTS, listCompanyForms } from 'Containers/Projects/actions';
...
export const createCompanyForm =
  (companyId, requestData) =>
  async (dispatch: any, _getState = null, utils: any) => {
    const response = await handleApiRequest(
      dispatch,
      utils.Api.post(`/contract-forms`, requestData));
  ...
  async (dispatch: any, _getState = null, utils: any) => {
    const response = await handleApiRequest(
      dispatch,
      utils.Api.delete(`/contract-forms/${id}`) );
